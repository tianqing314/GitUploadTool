...
public interface IGitService
{
    ...
    Task<bool> AddRemoteAsync(string path, string remoteUrl);
    Task<bool> PushAsync(string path, string branch = "main");
    Task<bool> HasRemoteAsync(string path);
    Task<string?> GetRemoteUrlAsync(string path);
    Task<List<UploadProgress>> UploadProjectAsync(string path, string remoteUrl, string branch, string commitMessage, IProgress<UploadProgress>? progress = null);
}
